...
The public API lets you drive your convix instance from your own
...
```
Authorization: Bearer convix_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** convix
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer convix_live_xxx"
```
...
Rather than polling, register an endpoint and convix will POST to it when
things happen in your account. **Migration required:** apply
`db/migrations/0001_init.sql` (tabla `webhook_endpoints`, heredada de la migración 028 original).
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; convix keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer convix_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/convix", "events": ["message.received"] }'
...
Headers: `X-Convix-Event`, `X-Convix-Webhook-Id`, and `X-Convix-Signature`.

### Verifying the signature

`X-Convix-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages convix stores (inbox + API sends), not broadcast-only
