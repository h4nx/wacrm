...
// Cliente con privilegios de servicio. Hace falta para detectar un
// phone_number_id ya reclamado por *otro* usuario — bajo RLS la sesión
// propia no ve filas ajenas, así que el conflicto sería invisible.
...
export async function POST(request: Request) {
  try {
    ...
    // convix is single-tenant-per-WhatsApp-number — letting two accounts
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one Convix user.',
        },
        { status: 409 }
      )
    }
