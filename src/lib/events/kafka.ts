 ...
 *
 * Cada evento de dominio de convix se publica en Kafka con la
 ...
 * esa variable es un no-op — convix sigue siendo autocontenido. La
...
function topicPrefix(): string {
  return process.env.KAFKA_TOPIC_PREFIX ?? 'convix';
}
  ...
  if (!producerPromise) {
    producerPromise = (async () => {
      ...
      const kafka: Kafka = new KafkaCtor({
        clientId: process.env.KAFKA_CLIENT_ID ?? 'convix',
...
/**
 * Publica `convix.<event>` en el bus del ecosistema. Best-effort:
